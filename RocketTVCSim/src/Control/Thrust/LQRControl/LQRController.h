...
public:
    ...
    */
    ...
    LQRController(Eigen::MatrixXd Ainput, Eigen::MatrixXd Binput,
        Eigen::MatrixXd Qinput, Eigen::MatrixXd Rinput, Eigen::MatrixXd Sinput);
...
*/
    ...
    void ComputeInitialGuess(Eigen::MatrixXd& initialGuessMatrix);
    ...
    Input arguments:
        ...
        Am^{T}*SolutionMatrix+SolutionMatrix*A-Rm
    */
    void SolveLyapunovEquation(const Eigen::MatrixXd& Am, const Eigen::MatrixXd& Rm, Eigen::MatrixXd& SolutionMatrix, double& residual);
    ...
    /*
        This function will apply the optimal control law to the system to acheive the optimal control output
    */
    Eigen::Matrix<double, -1, 1> computeOptimalControl(Eigen::Matrix<double, -1, 1> state, Eigen::Matrix<double, -1, 1> desired_state, double numIterations);
    ...
    */
    void SimulateSystem(Eigen::MatrixXd x0, int simulationTimeSteps, double h);
    ...
    */
    void SaveData(std::string KFile, std::string AclFile, std::string solutionRiccatiFile, std::string simulatedStateTrajectoryFile) const;


private:

    // MatrixXd is an Eigen typdef for Matrix<double, Dynamic, Dynamic>
    Eigen::MatrixXd A, B; // system matrices
    Eigen::MatrixXd Q, R, S;   // weighting matrices

    Eigen::MatrixXd K; // LQR control gain matrix 
    Eigen::MatrixXd Acl; // LQR closed-loop matrix Acl=A-B*K
    Eigen::MatrixXd solutionRiccati; // solution of the Riccati equation
    Eigen::MatrixXd In; // identity matrix for computations
    Eigen::MatrixXd simulatedStateTrajectory; // simulated state trajectory of the closed-loop state-space model
