...
// See the WndProc() function below for our to dispatch events to the Win32 backend.
			...
			if (ImPlot::BeginPlot("AutoTunePT2")) {
				ImPlot::SetupAxesLimits(0, 15, -10, 10);

				//  ImPlot::PlotBars("My Bar Plot", bar_data, 11);
				ImPlot::PlotLine("angular velocity", &simulationTime[0], &xAngularVelocity[0], simulationTime.size());
				ImPlot::PlotLine("autoTunerOut", &simulationTime[0], &pid2out[0], simulationTime.size());
				ImPlot::PlotLine("thrust Pitch", &simulationTime[0], &thrustParameter[0], simulationTime.size());
				ImPlot::PlotLine("x Angle", &simulationTime[0], &xAngle[0], simulationTime.size());
				ImPlot::EndPlot();
			}
			ImGui::End();
