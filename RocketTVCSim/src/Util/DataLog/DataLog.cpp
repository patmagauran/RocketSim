...
void DataLog::logData(std::string name, float value)
{
	initialize();
	currentData[name] = value;
}
void DataLog::pushTimestamp(float timestamp)
{
	initialize();
...
void DataLog::drawPlots() {
	DataRow currentRow;
	
	if (dataQueue.try_dequeue(currentRow)) {
		for (const auto& [name, value] : currentRow.data) {
			const auto& [index, inserted] = plotData.try_emplace(name, PLOT_DATA_TYPE(
		{ currentRow.timestamp }, { value }));
			if (!inserted) {
				plotData[name].first.push_back(currentRow.timestamp);
				plotData[name].second.push_back(value);
			}
		}
	}
